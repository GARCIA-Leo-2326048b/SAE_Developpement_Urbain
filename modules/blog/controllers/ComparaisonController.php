...
class ComparaisonController
{
    ...
    public function execute($filesSimName, $filesVerName, $experimentId = null)
    {
        if ($experimentId) {
            ...
            $geoJsonSimName = $experimentData['nom_sim'] ?? [];
            $geoJsonVerName = $experimentData['nom_ver'] ?? [];
            ...
            $experiment['nom_sim'] = isset($experiment['nom_sim']) ? json_decode($experiment['nom_sim'], true) : [];
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log('Erreur de décodage JSON pour nom_sim : ' . json_last_error_msg());
                $experiment['nom_sim'] = [];
            }

            $experiment['nom_ver'] = isset($experiment['nom_ver']) ? json_decode($experiment['nom_ver'], true) : [];
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log('Erreur de décodage JSON pour nom_ver : ' . json_last_error_msg());
                $experiment['nom_ver'] = [];
            }

            $this->view->setId($experimentId);
            $this->view->showComparison($formattedData, $experimentData['geoJsonSim'], $experimentData['geoJsonVer'], $geoJsonSimName, $geoJsonVerName, $charts);
        } else {
            $fileDataSim = [];
